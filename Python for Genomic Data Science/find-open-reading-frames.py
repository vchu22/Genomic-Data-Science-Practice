...
for k in fasta:
    ...
    print("Sequence ",k,": Num of forward segments ",len(forword_ORF)," , Reverse segments ",len(reverse_ORF))
    if (len(forword_ORF) > 0):
        r = (3,len(forword_ORF))[len(forword_ORF) < 3]
        for i in range(0,r):
            print("Fowards segments - Reading frame ",i+1,": ",forword_ORF[i])
    if (len(reverse_ORF) > 0):
        r = (3,len(reverse_ORF))[len(reverse_ORF) < 3]
        for i in range(0,r):
            print("Reverse segments - Reading frame ",i+1,": ",reverse_ORF[i])
    print